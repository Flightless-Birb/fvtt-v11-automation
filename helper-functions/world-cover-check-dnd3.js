...
Hooks.on("midi-qol.preAttackRollComplete", async (workflow) => {
    try {
        ...
        for (let t = 0; t < targets.length; t++) {
            ...
            // attack cover
            if (!(workflow.item.system.actionType == "rsak" && workflow.actor.flags["midi-qol"].spellSniper) && !(workflow.item.system.actionType == "mwak" && MidiQOL.computeDistance(workflow.token, token, false) < 10) && !((workflow.item.system.actionType == "rwak" || (workflow.item.system.actionType == "mwak" && workflow.item.system.properties.has("thr"))) && workflow.actor.flags["midi-qol"].sharpShooter)) {
                try {
...
Hooks.on("midi-qol.preCheckSaves", async (workflow) => {
    try {
        ...
        for (let t = 0; t < targets.length; t++) {
            ...
            // save cover
            if (workflow.item.system.save.dc && workflow.item.system.save.ability === "dex" && workflow.item.system.actionType !== "abil" && !["Sacred Flame"].find(i => workflow.item.name.includes(i))) {
                try {
                    const source = workflow.templateData ? { center: { x: workflow.templateData.x, y: workflow.templateData.y }, losHeight: workflow.templateData.flags?.levels?.elevation ?? 0 } : workflow.token;
