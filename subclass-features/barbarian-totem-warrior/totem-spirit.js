                ...
                let hook2 = Hooks.on("midi-qol.RollComplete", async (workflowNext) => {
                    if (args[0].uuid == workflowNext.uuid) {
                        ...
                        const effectData = {
                            ...
                            duration: { seconds: 60 },
                            flags: { ActiveAuras: { aura: "Enemy", displayTemp: false, height: true, hidden: true, hostile: false, ignoreSelf: true, isAura: true, onlyOnce: false, radius: 5, wallsBlock: "true", customCheck: "MidiQOL.typeOrRace(actor.uuid)" } }
                        }
