try {
    ...
    if (args[0].tag != "DamageBonus" || (!args[0].hitTargets.length && MidiQOL.configSettings().autoRollDamage == "always") || args[0].item.type != "spell" || !args[0].item.system.school || args[0].spellLevel == 0 || args[0].spellLevel > 5 || !["msak", "rsak", "save", "other"].includes(item.system.actionType) || args[0].damageRoll.terms.find(t => ["healing", "temphp", "midi-none", ""].includes(t.flavor.toLowerCase())) || !((item.flags?.["tidy5e-sheet"]?.parentClass.toLowerCase().includes("wizard") || item.system.chatFlavor).toLowerCase().includes("wizard") || (!item.flags?.["tidy5e-sheet"]?.parentClass && !item.system.chatFlavor && ["prepared", "always"].includes(item.system?.preparation?.mode)))) return;
