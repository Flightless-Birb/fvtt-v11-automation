try {
    if (args[0].macroPass != "postActiveEffects" || args[0].item.type != "spell" || args[0].item.system.school != "div" || args[0].item.system.level < 2 || !["prepared", "always", "pact"].includes(args[0].item.system.preparation.mode)) return;
    let options = "";
    Object.keys(args[0].actor.system.spells).forEach(key => {
        if (key === "pact" && args[0].actor.system.spells[key].level <= 5 && args[0].actor.system.spells[key].max > 0 && args[0].actor.system.spells[key].value < args[0].actor.system.spells[key].max) options += `<option id="${args[0].actor.system.spells.pact.level}" value="${key}">Pact Magic [Level ${args[0].actor.system.spells.pact.level}] (${args[0].actor.system.spells[key].value}/${args[0].actor.system.spells[key].max} Slots)</option>`;
        if (key !== "pact" && +key.slice(-1) <= 5 && args[0].actor.system.spells[key].max > 0 && args[0].actor.system.spells[key].value < args[0].actor.system.spells[key].max) options += `<option id="${key.slice(-1)}" value="${key}">Level ${key.slice(-1)} (${args[0].actor.system.spells[key].value}/${args[0].actor.system.spells[key].max} Slots)</option>`;
    });
...
} catch (err)  {console.error("Expert Divination Macro - ", err)}