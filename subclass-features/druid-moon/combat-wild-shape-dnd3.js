try {
    ...
    let newDamageFormula = `${+slot.level}d8`;
    args[0].workflow.damageRolls[0] = await new Roll(newDamageFormula).roll();
    args[0].workflow.damageTotal = workflow.damageRolls[0].total;
    args[0].workflow.damageRollsHTML = await workflow.damageRolls.render();
} catch (err)  {console.error("Combat Wild Shape Macro - ", err)}