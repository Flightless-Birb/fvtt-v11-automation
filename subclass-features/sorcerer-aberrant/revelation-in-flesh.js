try {
	...
	await new Promise((resolve) => {
		new Dialog({
			...
			<script>
				var limit = ${uses};
				$("input[type='checkbox'][name='consumeCheckbox']").change(function() {
					var bol = $("input[type='checkbox'][name='consumeCheckbox']:checked").length >= limit;
					$("input[type='checkbox'][name='consumeCheckbox']").not(":checked").attr("disabled", bol);
				});
			</script>
			`,
			buttons: {
