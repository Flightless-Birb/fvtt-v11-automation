try {
    if (args[0].macroPass == "postActiveEffects" && (args[0].hitTargets.length || MidiQOL.configSettings().autoRollDamage != "always") && args[0].damageRolls && ["mwak", "rwak", "msak", "rsak"].includes(args[0].item.system.actionType) && args[0].actor.effects.find(e => e.name == "Rage") && (!game.combat || game.combat?.current?.tokenId == args[0].tokenId) && (!game.combat || !args[0].actor.effects.find(e => e.name == "Used Ancestral Protectors" && !e.disabled))) {
        const effectData1 = {
