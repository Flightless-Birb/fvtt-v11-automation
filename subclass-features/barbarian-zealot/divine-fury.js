try {
    if (args[0].workflow.divineFury) {
        ...
        let bonusRoll = await new Roll('0 + ' + `${diceMult}d6[${args[0].workflow.divineFury}] + ${level}[${args[0].workflow.divineFury}]`).evaluate({async: true});
        for (let i = 1; i < bonusRoll.terms.length; i++) {
            args[0].damageRoll.terms.push(bonusRoll.terms[i]);
        }
        args[0].damageRoll._formula = args[0].damageRoll._formula + ' + ' + `${diceMult}d6[${args[0].workflow.divineFury}] + ${level}[${args[0].workflow.divineFury}]`;
    ...
    args[0].damageRoll._total = args[0].damageRoll.total + bonusRoll.total;
    await args[0].workflow.setDamageRoll(args[0].damageRoll);
    args[0].workflow.divineFury = damageType;
