try {
    if (args[0].macroPass == "preambleComplete" && (["undead", "fiend"].find(c => args[0].targets[0].actor.system.details?.race?.toLowerCase()?.includes(c)) || ["undead", "fiend"].find(c => args[0].targets[0].actor.system.details?.type?.value?.toLowerCase()?.includes(c)))) {
        const effectData = {
        ...
        await MidiQOL.socket().executeAsGM("createEffects", { actorUuid: args[0].targets[0].actor.uuid, effects: [effectData] });
    } else if (args[0].macroPass == "preTargetSave" && args[0].workflow.saveDetails && args[0].item.name.toLowerCase().includes("abjure enemy")) {
