    try {
        if (args[0].macroPass != "preStart" || args[0].workflow.awakenedSpellbook || args[0].item.type != "spell" || !args[0].item.system.school || args[0].spellLevel == 0 || !["msak", "rsak", "save", "other"].includes(item.system.actionType) || !args[0].item.system.damage.parts.find(p => ["acid", "bludgeoning", "cold", "fire", "force", "lightning", "necrotic", "piercing", "poison", "psychic", "radiant", "slashing", "thunder"].includes(p[1].toLowerCase())) || !(((item.flags?.["tidy5e-sheet"]?.parentClass.toLowerCase().includes("wizard")) || (item.system.chatFlavor.toLowerCase().includes("wizard"))) || (!item.flags?.["tidy5e-sheet"]?.parentClass && !item.system.chatFlavor && ["prepared", "always"].includes(item.system?.preparation?.mode)))) return;
