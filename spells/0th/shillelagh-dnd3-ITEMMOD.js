try {
    ...
    if (lastArg.macroPass == "postActiveEffects") {
        const equipped = actor.items.filter(i => i.type == "weapon" && i.system.equipped && ["quarterstaff", "club"].includes(i.system.type.baseItem));
        if (!equipped.length) {
        ...
        const effectData = {
            ...
            changes: [{ key: "macro.execute.GM", mode: 0, value: `Compendium.dnd-5e-core-compendium.macros.XTAfGLeuiLA2FmTV ${weaponId}`, priority: 20 }],
            duration: { seconds: 60 },
            origin: lastArg.uuid,
            disabled: false,
            isSuppressed: false
        ...
        if (weapon.flags["midi-qol"].tempSystem) await weapon.setFlag("midi-qol", "tempSystem", weapon.flags["midi-qol"].tempSystem.concat([{ source: "shillelagh", id: lastArg.efData._id, properties: ["mgc"], ability: `${actor.system.attributes.spellcasting ? actor.system.attributes.spellcasting : "int"}`, dieReplace: ["d(4|6)", "d8"] }]));
        if (!weapon.flags["midi-qol"].tempSystem) await weapon.setFlag("midi-qol", "tempSystem", [{ source: "core", id: weapon.id, system: JSON.parse(JSON.stringify(weapon.system)) }, { source: "shillelagh", id: lastArg.efData._id, properties: ["mgc"], ability: `${actor.system.attributes.spellcasting ? actor.system.attributes.spellcasting : "int"}`, dieReplace: ["d(4|6)", "d8"] }]); 
        await weapon.setFlag("midi-qol", "shillelagh", lastArg.efData._id);
        await weapon.update({
            name: weapon.name + " (Shillelagh)",
            system: { ability: `${actor.system.attributes.spellcasting ? actor.system.attributes.spellcasting : "int"}`, properties: new Set([...weapon.system.properties].concat(["mgc"])).reduce((obj, str) => ({...obj, [str]: true}), {}), "damage.parts": parts}
        });
    } else if (args[0] == "off") { 
        let weapon = actor.items.find(i => i.flags["midi-qol"].shillelagh == lastArg.efData._id);
