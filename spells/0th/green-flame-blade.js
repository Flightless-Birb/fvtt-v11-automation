try {
	...
	if (!attackWorkflow.hitTargets.size && MidiQOL.configSettings().autoRollDamage == "always") return;
	new Dialog({
		...
		buttons: {
			Ok: {
				...
				callback: async () => { 
					...
					if (MidiQOL.computeDistance(attackWorkflow.targets.values().next().value, target, false) > 5) return ui.notifications.warn(`Target too far away (${MidiQOL.computeDistance(attackWorkflow.targets.values().next().value, target, false)} Feet)`);
					...
					await MidiQOL.completeItemUse(damageItem, {}, { showFullCard: true, createWorkflow: true, configureDialog: false, targetUuids: [target.uuid] });
				},
			},
			Cancel: { label: "Cancel" },
		},
