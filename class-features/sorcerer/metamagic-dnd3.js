try {
    ...
    if (args[0].macroPass == "preTargeting" && usesItem.system.uses.value && ["action", "bonus", "reaction", "reactiondamage", "reactionmanual"].includes(args[0].item.system.activation.type) && !args[0].workflow.metamagic) {
    ...
    } else if (args[0].macroPass == "preDamageRollComplete" && (args[0].hitTargets.length || MidiQOL.configSettings().autoRollDamage != "always") && args[0].actor.items.find(i => i.name == "Metamagic: Empowered Spell") && usesItem.system.uses.value && args[0].item.system.damage?.parts?.length && !["healing", "temphp", "", "midi-none"].includes(args[0].item.system.damage.parts[0][1]) && args[0].damageRolls) {
        // empowered spell
        ...
        for (let d = 0; d < args[0].workflow.bonusDamageRolls?.length; d++) {
            let terms = args[0].bonusDamageRoll.terms;
            for (let t = 0; t < terms?.length; t++) {
                if (!terms[t].faces) continue;
                let results = terms[t].results;
                for (let r = 0; r < results.length; r++) {
