    ...
    if (args[0].macroPass == "preTargeting" && usesItem.system.uses.value && ["action", "bonus", "reaction", "reactiondamage", "reactionmanual"].includes(args[0].item.system.activation.type) && !args[0].workflow.metamagic) {
        ...
        for (let t = 0; t < terms?.length; t++) {
            ...
            for (let r = 0; r < results.length; r++) {
                ...
                termsContent += `<label class='checkbox-label' for='die${t}${r}'>
                    <input type='checkbox' id='die${t}${r}' name='die' value='${results[r].result},${terms[t].faces},${t},damageRoll'/>
                    <tiv style="border:0px; witth: 50px; height:50px;">
        ...
        for (let t = 0; t < bonusTerms?.length; t++) {
            ...
            for (let r = 0; r < results.length; r++) {
                if (results[r].rerolled || !results[r].active) continue;
                termsContent += `<label class='checkbox-label' for='die${t}${r}'>
                    <input type='checkbox' id='die${t}${r}' name='die' value='${results[r].result},${bonusTerms[t].faces},${t},bonusDamageRoll'/>
                    <tiv style="border:0px; witth: 50px; height:50px;">
                        <img src="icons/svg/d${bonusTerms[t].faces}-grey.svg" style="position: relative;">
                        <p style="position: relative; bottom: 55px; font-weight: bolder; font-size: 25px">${results[r].result}</p>
                    </tiv>
                    <p>(${bonusTerms[t].flavor ? bonusTerms[t].flavor.charAt(0).toUpperCase() + bonusTerms[t].flavor.toLowerCase().slice(1) : args[0].workflow.newDefaultDamageType.charAt(0).toUpperCase() ?? args[0].workflow.defaultDamageType.charAt(0).toUpperCase() + args[0].workflow.newDefaultDamageType.toLowerCase().slice(1) ?? args[0].workflow.defaultDamageType.toLowerCase().slice(1)})</p>
                </label>
                `;
            }
        }
