try {
    ...
    if (args[0].macroPass == "preTargeting" && usesItem.system.uses.value && ["action", "bonus", "reaction", "reactiondamage", "reactionmanual"].includes(args[0].item.system.activation.type) && !args[0].workflow.metamagic) {
        ...
        if (metamagic == "quickened") {
            ...
            let hook3 = Hooks.on("midi-qol.RollComplete", async (workflowNext) => { // update effects on templates to use new damage type
                if (workflowNext.item.uuid == args[0].item.uuid && workflowNext.templateId && args[0].item.effects.find(e => e.flags?.ActiveAuras?.isAura && e.changes.find(c => options.find(o => c.value.toLowerCase().includes(o.toLowerCase()))))) {
                    let template = canvas.templates.get(workflowNext.templateId);
                    if (!template || !template?.document?.flags?.ActiveAuras?.IsAura) return;
                    template?.document?.flags?.ActiveAuras?.IsAura?.forEach(async (a) => { a?.data?.changes?.forEach(async (c) => { options.forEach(o => c.value = c.value.replace(o.toLowerCase(), type.toLowerCase())) }) });
                    //console.error(template)
                    //let auras = [...template?.document?.flags?.ActiveAuras?.IsAura];
                    //if (!auras) return;
                    //console.error(template?.document?.flags?.ActiveAuras?.IsAura)
                    //auras.forEach(async (a) => {
                    //    changes = [...a?.data?.changes];
                    //    changes.forEach(c => options.forEach(o => c.value = c.value.replace(o.toLowerCase(), type.toLowerCase())));
                    //    a.data.changes = changes;
                    //});
                    //console.error(auras)
                    //await template.document.setFlag("ActiveAuras", "IsAura", auras);
                    //let changes = [...template?.document?.flags?.ActiveAuras?.IsAura?.[0]?.data?.changes];
                    //console.error(changes)
                    //changes.forEach(c => options.forEach(o => c.value = c.value.replace(o.toLowerCase(), type.toLowerCase())));
                    //template?.document?.flags?.ActiveAuras?.IsAura?.[0]?.data?.changes = changes;
                    //console.error(template?.document?.flags?.ActiveAuras?.IsAura)
                    //await MidiQOL.socket().executeAsGM("updateEffects", { actorUuid: effect.parent.uuid, updates: [{ _id: effect._id, changes: changes }] });
                }
            });
            let hook4 = Hooks.on("midi-qol.preTargeting", async (workflowNext) => {
                if (workflowNext.item.uuid == args[0].item.uuid && workflowNext.metamagic != "extended") {
                    Hooks.off("midi-qol.preDamageRollComplete", hook1);
                    Hooks.off("createActiveEffect", hook2);
                    Hooks.off("midi-qol.postApplyDynamicEffects", hook3);
                    Hooks.off("midi-qol.preTargeting", hook4);
                }
            });
            if (consume) await usesItem.update({ "system.uses.value": Math.max(0, usesItem.system.uses.value - 1) });
