try {
    ...
    if (args[0].tag != "DamageBonus" || (!args[0].hitTargets.length && MidiQOL.configSettings().autoRollDamage == "always") || !["mwak", "rwak"].includes(args[0].item.system.actionType) || args[0].disadvantage || (args[0].item.system.actionType == "mwak" && !args[0].item.system.properties.has("fin")) || (game.combat && args[0].actor.effects.find(e => e.name == "Used Sneak Attack" && !e.disabled)) || !(args[0].advantage || canvas.tokens.placeables.find(t => t.actor && MidiQOL.typeOrRace(t.actor) && t !== args[0].workflow.token && t !== args[0].targets[0] && t?.document?.disposition == args[0].workflow.token?.document?.disposition && !MidiQOL.checkIncapacitated(t.actor) && MidiQOL.computeDistance(t, args[0].targets[0], false) < 10))) return;
    let useFeat = true;
    if (game.combat) {
